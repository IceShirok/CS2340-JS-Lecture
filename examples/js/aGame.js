...
function randomize() {
    ...
    //$("#random").text(" Testing pageX: " + startWidth + ", pageY: " + startHeight );
}
...
function tellIfClose(x, y) {
    ...
    $("#test").text(x + " " + y);
    //document.getElementById("test").innerText = tellIfClose(x + " " + y);
    if (isClose > 550) {
        return "Cold!";
    } else if (isClose > 200) {
        return "Lukewarm...";
...
$(document).on("mousemove", function(event) {
    ...
    // document.getElementById("howClose").innerText = tellIfClose(event.pageX, event.pageY);
});
...
$(document).on("click", function(event) {
    if(event.pageX>startWidth && event.pageX<startWidth+squareSize
        ...
        // document.getElementById("message").innerText = "Congratulations! You've found the hidden block!";
    }
});
